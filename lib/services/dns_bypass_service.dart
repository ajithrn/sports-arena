...
/// 1. Client sends: CONNECT streamfree.top:443 HTTP/1.1
/// 2. Proxy resolves streamfree.top via DoH → gets unblocked IP
/// 3. Proxy connects to that IP on port 443
/// 4. Proxy responds: HTTP/1.1 200 Connection established
/// 5. Client does TLS handshake through the tunnel (SNI = streamfree.top)
...
class DnsBypassService {
  ...
  Future<void> _handleConnect(HttpRequest request) async {
    final target = request.uri.toString(); // e.g., "streamfree.top:443"
